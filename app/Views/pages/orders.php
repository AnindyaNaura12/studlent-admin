...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card.completed .stat-icon { background: #dbeafe; color: #3b82f6; }
        ...
        .progress-bar { height: 6px; background: #fff1e6; border-radius: 3px; }
        .progress-fill { height: 6px; border-radius: 3px; background: #f97316; }
        .progress-text { font-size: 11px; color: #9ca3af; margin-top: 3px; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?= BASE_URL ?>order" class="nav-item active"><i class="ti ti-shopping-cart"></i> Orders</a>
...
<main class="main">
    <div class="topbar">
        <div>
            <h1>Orders</h1>
            <p>Kelola semua transaksi order Studlent</p>
        </div>
    ...
    <div class="panel">
        <div class="panel-header">
            <span class="panel-title">Daftar Orders</span>
            <span class="total-badge"><?= count($orders) ?> order</span>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php if (!empty($orders)): ?>
                    ...
                    <tr>
                        <td style="font-family:monospace;font-size:12px">#<?= htmlspecialchars(substr($order['id_order'], 0, 8)) ?></td>
                        <td style="font-family:monospace;font-size:12px"><?= htmlspecialchars(substr($order['id_client'] ?? '-', 0, 8)) ?></td>
                        <td style="font-family:monospace;font-size:12px"><?= htmlspecialchars(substr($order['id_freelancer'] ?? '-', 0, 8)) ?></td>
                        <td><?= !empty($order['deadline']) ? date('d M Y', strtotime($order['deadline'])) : '-' ?></td>
                        <td>
                        ...
                        <td>
                            <a href="<?= BASE_URL ?>order/detail?id=<?= urlencode($order['id_order']) ?>" class="btn-detail">
